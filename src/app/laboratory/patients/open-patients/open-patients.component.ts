...
@Component({
  ...
  templateUrl: './open-patients.component.html',
  styleUrls: ['./open-patients.component.sass'],
})
export class OpenpatientsComponent implements OnInit {
  displayedColumns = [
    'select',
    'patient_ID',
    'sortDate',
  ...
  exampleDatabase: PatientsLabService | null;
  ...
  constructor(
    ...
    public patientsService: PatientsLabService,
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        ...
        this.exampleDatabase.dataChange.value.unshift(
          this.patientsService.getOpenpatients()
        );
        this.refreshTable();
  ...
  editCall(row) {
   /** console.log(this.id);
    ...
    window.localStorage.removeItem('patient_id_lab')
    window.localStorage.setItem('patient_id_lab',row.patient_ID)
    window.localStorage.removeItem('visit_id5')
    window.localStorage.setItem('visit_id5',row.v);
    console.log(window.localStorage.getItem('visit_id5'))
    this.router.navigateByUrl('/laboratory/form');
  }
  ...
  public loadData() {
    this.exampleDatabase = new PatientsLabService(this.httpClient);
...
export class ExampleDataSource extends DataSource<Patient> {
  ...
  constructor(
    public _exampleDatabase: PatientsLabService,
  ...
  connect(): Observable<Patient[]> {
    ...
    this._exampleDatabase.getInprogressPatientsLabTo();
